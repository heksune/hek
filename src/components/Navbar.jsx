...
export default function Navbar() {
    ...
    const navElements = [
        { name: "Home", link: "/" },
        { name: "Projects", link: "/projects", disabled: true },
        { name: "Blog", link: "/blog", disabled: true },
        { name: "Contact", link: "/contact", disabled: true }
    ];
        ...
        >
            ...
            >
                {navElements.map((navElement, index) => (
                    <li
                        key={index}
                        className={`relative text-sm font-sans font-medium duration-150 ${
                            pathname === navElement.link ? "text-white" : "text-zinc-400 hover:text-white"
                        } ${navElement.disabled ? "opacity-50 cursor-not-allowed" : ""}`}
                    >
                        {navElement.disabled ? (
                            <span className="relative px-2 py-2">{navElement.name}</span>
                        ) : (
