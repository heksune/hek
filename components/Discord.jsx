...
export default function Discord() {
  ...
  const codeActivity = data.data.activities.find((activity) =>
      ...
      {largeImageUrl ? (
        <a className="rounded-lg p-4 border-neutral-800 border-[1px] bg-neutral-850">
          <div className="flex items-center space-x-4">
            ...
            />
            <div>
              <h className="font-medium text-black dark:text-white">
                {codeActivity ? codeActivity.name : "No Code Activity"}
              </h>
              <h2 className="font-medium text-xs text-white/50">
                {codeActivity ? codeActivity.details : ""}
              </h2>
              <p className="font-medium text-xs text-white/30 line-clamp-4">
                {codeActivity ? codeActivity.state : ""}
              </p>
            </div>
          </div>
        </a>
